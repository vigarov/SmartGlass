...
Header::Header(std::string& s,unsigned char hours , unsigned char minutes ,bool overwrites, pixel_pair_t offsets , unsigned char animate, unsigned char priority,TaskHandle_t notifyOnDraw):
    ...
    m_bleIcon_p(std::make_shared<BLEIcon>(s+"BLE Icon",overwrites,offsets + (pixel_pair_t){BLEICON_OFFSET_X,0},animate,priority))
    {
    ...
    tempS = ":";
    m_displayables.push_back(std::make_shared<RawText>(tempS,overwrites,offsets+(pixel_pair_t){COLON_OFFSET_X,0},animate,priority));
    m_displayables.push_back(m_minuteText_p);
    tempS = s+"bell";
    m_displayables.push_back(std::make_shared<Bell>(tempS,overwrites,offsets+(pixel_pair_t){BELL_OFFSET_X,0},animate,priority));
