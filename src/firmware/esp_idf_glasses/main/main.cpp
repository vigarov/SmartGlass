...
class GlobalsManager {
    private:
    BLEUUID servicesUUID[NB_SERVICES] = {BLEUUID(NOTIF_SERVICE_UUID),BLEUUID((uint16_t)BATTERY_SERVICE_UUID)};
    ...
    BLEService* get_svc(svc_idx_e svc_idx) {
        if (!this->initialised) {
            ESP_LOGE(TAG, "BLE service not initialised");
        }
...
void createTask(TaskFunction_t pvTaskCode, const char *const pcName, const uint32_t usStackDepth, void* const pvParameters, UBaseType_t uxPriority, const BaseType_t xCoreID, size_t idx){
    ...
    int error = pdPASS;
    if((error = xTaskCreatePinnedToCore(pvTaskCode, pcName, usStackDepth, pvParameters,uxPriority,allTasks[idx],xCoreID)) != pdPASS
        || allTasks[T_BLE] == NULL){
/*        #if ARDUINO_IDE == 1
...
//#endif
{
    ...
    createTask(T_HandleBLE,"BLEHandler",10240,(void* const)&glob_mgr,configMAX_PRIORITIES-1,PRO_CPU,T_BLE);
